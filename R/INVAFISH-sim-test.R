...
for(sec_disp in c(FALSE, TRUE)) {
  ...
  for(exterminate in c(FALSE, TRUE)) {
  ...
  # }
  if(focal_speciesid==26181) { # gjedde
    #exterminate specific lakes
    exwaterbodyID <- 4118446

  } else if(focal_speciesid==26436) { # solabbor
    #exterminate specific lakes
    exwaterbodyID <- inndata_sim1$waterBodyID[inndata_sim1[focal_species_str]==1]

  } else if(focal_speciesid==26138) { # ørekyt
    #exterminate specific lakes
    exwaterbodyID <- 4110252
    fish_barrier  <- append(as.integer(strsplit(dbGetQuery(con, paste0('SELECT upstream_lakes FROM fremmedfisk.fremmedfisk_lake_connectivity_summary WHERE "lakeID" = ', exwaterbodyID, ';'))$upstream_lakes, ',')[[1]]), exwaterbodyID)
  }
    ...
    if(focal_speciesid==26436) { # solabbor
    ...
    geoselect_no_occ_pop_5000 <- dbGetQuery(con, paste0('SELECT al.id AS "waterBodyID", count(ol.geom) AS n_pop FROM
                                                  (SELECT id, geom FROM nofa.lake WHERE county IN (', paste0("\'", gsub(", ", "\', \'", toString(unique(counties[,1]))), "\'"), ')) AS al,
                                           (SELECT geom FROM nofa.lake WHERE id IN (', toString(unique(inndata_sim1$waterBodyID[inndata_sim1[,focal_species_str]==1])), ')) AS ol
                                           WHERE ST_DWithin(al.geom, ol.geom, 5000)
                                           GROUP BY al.id'))
  ...
  for(slope_barrier in barriers) { # max slope for migration upstream
        ...
        if(with_secondary==TRUE && length(tmp_trans[tmp_trans$introduced==1,]$waterBodyID)>0){
          ...
          if(use_slope_barrier==TRUE){
            ...
            if(length(reachable_lakes_species)>0){
              ...
              if(exterminate & length(fish_barrier) > 0) {
    ...
    tmpout[["focal_species"]] <- focal_species
    tmpout[["time_slot_length"]] <- time_slot_length
    tmpout[["start_year"]] <- start_year
    tmpout[["secondary_dispersal"]] <- sec_disp
    ...
    dbWriteTable(con, c(conmat_schema,
                        paste0("sim_out_lake_",
                               tolower(focal_species_str),
                               ifelse(exterminate,"_with_ext_","_without_ext_"),
                               Nsims,
                               "simu_",
                               ifelse(sec_disp, paste0(slope_barrier, "_deg"),
                                      "no_secondary_dispersal")
                               )
